...
const PostList: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"recommendations" | "following">(
    "recommendations"
  );
  ...
  return (
    <div className="max-w-2xl mx-3">
      ...
      {posts.map((post) => (
        <div
          key={post.id}
          className="flex p-3 bg-stone-50 dark:bg-zinc-900 rounded-lg mb-2"
        >
          <div className="mr-2">
            <a
              href={`user/${post.user_id}`}
              className="block w-12 h-12 relative"
            >
              <Image
          ...
          <div className="w-full">
            <div className="flex items-center">
              <a href={`user/${post.user_id}`} className="font-semibold">
                カヲル
              </a>
